...
```{r}
?plot # Let's find out a bit about how to make plots
plot(0:30, dpois( x=0:30, lambda=15 ))
  # range of x values should be from 0:30
##the distribution on this looks right
new.name<- dpois( x=0:30, lambda=15 )
hist(new.name)
##this is plotting the probabilities as the numbers
  #make a histogram of the distribution
##hist may be the wrong fxn. THis works with barplot()
```


```{r}
#First make the distribution to use
##Define variables
x<-0:30
l<-15
##dpos makes a density fxn, gives the probability of each number
dpois(x=x ,lambda=l)
##put this into a vector
probabilities<- dpois(x=x , lambda=l)
#then you plot this
...
```{r}
?hist
hist(w, xlim = c(0, 30)) #plot the results putting a limit from 0 to 30 on x
```

Problems:

- I don't think I am doing the first histogram right, it looks very different form the 104 sample one
- Plan is to make the plots prettier before uploading 